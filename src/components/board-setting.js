...
class BoardSetting extends React.Component {
  onModifySettings = (selectedOption) => {
    ...
    if (setting_key === SETTING_KEY.TABLE_NAME) {
      updated = {
        _id: boardSetting._id,
        name: boardSetting.name,
        [setting_key]: value,
      };
    } else {
  ...
  getSelectorColumns = (columns) => {
    const {
      columnIconConfig,
      supportGroupbyColumnTypes,
      unsupportedSetTitleFieldTypes,
    } = this.props.dtableValue;
    let groupbyColumns = [],
      titleColumns = [];
    columns &&
  ...
  renderSelector = (options, settingKey) => {
    ...
    let selectedOption = options.find(
      (item) => item.value === boardSetting[settingKey]
    );
    if (
      !selectedOption &&
  ...
  getSelectorOptions(selectedTable, { groupbyColumns, titleColumns }) {
    ...
    const groupbyColumnOptions = this.createOptions(
      groupbyColumns,
      SETTING_KEY.GROUPBY_COLUMN_NAME,
      'value'
    );
    const titleColumnOptions = this.createOptions(
      titleColumns,
      SETTING_KEY.TITLE_COLUMN_NAME,
      'value'
    );
    return {
      tableOptions,
  ...
  moveColumn = (targetColumnKey, targetIndexColumnKey) => {
    ...
    const targetColumn = configuredColumns.filter(
      (column) => column.key == targetColumnKey
    )[0];
    const originalIndex = configuredColumns.indexOf(targetColumn);
    const targetIndexColumn = configuredColumns.filter(
      (column) => column.key == targetIndexColumnKey
    )[0];
  ...
  render() {
    ...
    const {
      ...
      groupbyColumnOptions,
      titleColumnOptions,
    } = this.getSelectorOptions(selectedTable, {
      groupbyColumns,
      titleColumns,
    });
    ...
    return (
      <div
        className='plugin-kanban-board-setting'
        style={{ zIndex: zIndexes.BOARD_SETTING }}
      >
